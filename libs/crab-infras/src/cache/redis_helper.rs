...
// use bb8_redis::redis;
// use bb8_redis::redis::{Value, cmd, PubSub};
...
//     pub async fn subscribe_channel(&self, channel: &str) -> Result<Receiver<String>> {
//         self.redis_cache.subscribe(channel).await
//     }
//     pub async fn subscribe_multiple(&self, channels: Vec<String>) -> Result<PubSub> {
...
//     }
//
//     // pub async fn create_message_receiver(&self, channels: Vec<String>) -> Result<broadcast::Receiver<RedisMessage>> {
//     //     let self_clone = Arc::new(self.clone());  // 使用 Arc 克隆 self
//     //
//     //     let receiver = self_clone.sender.subscribe();
//     //
//     //     tokio::spawn({
//     //         async move {
//     //             let self_clone = Arc::clone(&self_clone); // 在任务内部再次克隆 Arc
...
//         // 获取 PubSub 连接
//         let mut pubsub = self.subscribe_multiple(channels).await?;
//         loop {
//             // 从 PubSub 连接中读取消息
...
//
//             // // 解析消息
//             // if let Ok(message) = parse_redis_message(&msg) {
//             //     // 尝试将消息发送给监听者
